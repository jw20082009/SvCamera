package com.wilbert.svcamera.glutils2;
...
public class STGLRender {
     ...
     * @param textureId [in] OES的纹理id
     * @param buffer  [out] RGBA的buffer
     * @param bufIndex [in] 使用的buffer索引，使用{@link }时有效
     * @return 转换后的GL_TEXTURE_2D的纹理id
     */
    ...
    public void saveTextureToFrameBuffer(int textureOutId, ByteBuffer buffer) {
        if(mSavePictureFrameBuffers == null) {
            return;
        }
        GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, mSavePictureFrameBuffers[0]);
        GLES20.glViewport(0, 0, mViewPortWidth, mViewPortHeight);
